...
\section{AI-Lingua Benchmark on LibriSpeech}

    \subsection{System Overview}
    
        AI-Lingua integrates a Python FastAPI service (Gemini 2.5) with a Java Spring Boot orchestrator. The Java service loads or extracts subtitles, batches lines, requests CEFR-level simplification, segments scenes, and emits simplified, previews, and recaps outputs. A research module provides corpus loaders and metrics for benchmarking.
    
    \subsection{Dataset and Preparation}
    
        We evaluate on LibriSpeech (English audiobook speech). Transcripts are read from \texttt{*.trans.txt} files and aligned with \texttt{.flac} audio. Local files enable reproducible runs without network dependence.
    
    \subsection{Methods}
    
        Each human transcript segment is simplified to a target CEFR level while preserving meaning. For benchmarking, we treat the transcript for each sample as a single unit to evaluate semantic preservation and readability independently of alignment concerns.
    
    \subsection{Evaluation Metrics}
    
        We compute BERTScore (semantic fidelity), Flesch–Kincaid Grade (readability), and perplexity (fluency). Reports include per-sample details and aggregate statistics.
    
    \subsection{Experimental Setup}
    
        The research runner loads samples, invokes simplification at level A2, aggregates outputs, computes metrics, and saves a JSON report under \texttt{benchmark\_results/}. To mitigate API limits, runs use small subsets and include retry and delay policies.
    
    \subsection{Reproducibility}
    
        Data preparation and evaluation scripts are versioned with the codebase. The benchmark runner reads locally generated SRT/WAV pairs when available, falling back to online loaders only if necessary.
